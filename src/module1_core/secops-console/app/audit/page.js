  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        {breakGlassState === "IDLE" ? (
          <button
            ...
            className="btn-glass btn-glass-danger font-mono text-xs uppercase shadow-[0_0_20px_rgba(239,68,68,0.2)] animate-pulse"
          >
            🔥 Trigger Break-Glass
          </button>
        ) : (
          <div className="px-4 py-2 bg-rose-950/20 text-rose-400 border border-rose-500/30 rounded font-mono text-xs font-bold uppercase">
            🔥 BYPASS ACTIVE
          </div>
        )}
      </div>
      ...
      {breakGlassState === "TRIGGERED" && (
        <div className="glass-panel p-6 border-glow-danger bg-rose-950/5 space-y-4">
          <h2 className="text-lg font-bold text-rose-400 font-mono">🚨 EMERGENCY BREAK-GLASS BYPASS STATUS ACTIVE</h2>
          <div className="bg-black/60 border border-slate-900 rounded p-4 font-mono text-xs text-rose-300 space-y-2 max-h-[200px] overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-panel p-6 space-y-6">
          <div>
            <h2 className="text-lg font-bold text-slate-200">GUI Web Console Audit Stream</h2>
            <p className="text-xs text-slate-400">Captures routing updates and administrative operations</p>
          </div>

          <div className="space-y-4">
            {guiAudits.map((audit) => (
              <div key={audit.id} className="p-4 rounded-lg bg-black/30 border border-slate-800 flex flex-col gap-2">
                <div className="flex justify-between items-center text-[10px] font-mono">
                  <span className="text-cyan-400">Time: {audit.time} | User: {audit.user}</span>
                  <span className={`px-2 py-0.5 rounded font-bold ${
                    audit.status === "ALERT" ? "bg-rose-950/30 text-rose-400 border border-rose-500/20" : "bg-cyan-950/30 text-cyan-400 border border-cyan-500/20"
                  }`}>
                    {audit.status}
                  </span>
                </div>
                <p className="text-xs font-semibold text-slate-300">{audit.action}</p>
                <span className="text-[10px] font-mono text-slate-500">Route path: {audit.route}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-panel p-6 space-y-6">
          <div>
            <h2 className="text-lg font-bold text-slate-200">Host OS CLI Audit Stream (auditd)</h2>
            <p className="text-xs text-slate-400">Captures hardware hypervisor shells and container runtimes</p>
          </div>

          <div className="space-y-4">
            {cliAudits.map((audit) => (
              <div key={audit.id} className="p-4 rounded-lg bg-black/30 border border-slate-800 flex flex-col gap-2 font-mono">
                <div className="flex justify-between items-center text-[10px]">
                  <span className="text-slate-500">Host: {audit.host} | Process: {audit.process}</span>
                  <span className="text-cyan-400 font-bold">{audit.time}</span>
                </div>
                <pre className="p-2.5 bg-black/50 border border-slate-900 rounded text-cyan-300 text-xs break-all whitespace-pre-wrap select-all">
                  {audit.command}
                </pre>
              </div>
            ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="glass-panel max-w-md w-full p-6 border-glow-danger space-y-6">
            <div className="space-y-2 text-center">
              ...
              <h2 className="text-xl font-extrabold text-rose-500 uppercase tracking-widest font-mono">
            ...
            <p className="p-3.5 bg-rose-950/15 border border-rose-500/20 text-rose-300 rounded font-mono text-[10px] leading-relaxed">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex-1 btn-glass btn-glass-danger justify-center text-xs py-2.5 font-bold uppercase tracking-wider font-mono shadow-[0_0_15px_rgba(239,68,68,0.4)]"
              >
                Confirm Override
              </button>
            </div>
